...
public class PhoneBook {
    ...
    /** Function gets contact from user -
     Into the loop I'm checking if the name already exist. **/
    public void AddContact(Contact c) {
        flag = false;
        if (PhoneBook.size() > 0) {
            for (int i = 0; i < PhoneBook.size(); i++)
                if ((PhoneBook.get(i).getFirstName().equals(c.getFirstName())) && (PhoneBook.get(i).getMiddleName().equals(c.getMiddleName())) && (PhoneBook.get(i).getLastName().equals(c.getLastName()))) {
                    System.out.println("Contact already exist! Contacts must have different names.");
                    flag = true;
                    break;
                }
        }
    ...
    public void DeleteContactByName(String name) {
        ...
        if (PhoneBook.size() > 0) {
            for (int i = 0; i < PhoneBook.size(); i++)
                if ((PhoneBook.get(i).getFullName().equals(name))) {
                    PhoneBook.remove(i);
                    flag = true;
                    break;
                }
    ...
    public void SortPhonebookAscending() {
        if (temp.size() > 0) {
            ...
            Collections.sort(temp, (c1, c2) -> c1.getFirstName().compareTo(c2.getFirstName()));
    ...
    public void SortPhonebookDescending() {
        if (temp.size() > 0) {
            System.out.println(temp);
            Collections.sort(temp, Collections.reverseOrder((c1, c2) -> c1.getFirstName().compareTo(c2.getFirstName())));
            System.out.println(temp);
            temp = PhoneBook;
        } else System.out.println("PhoneBook is empty!!");
    }
        ...
        if (PhoneBook.size() > 0) {
            ...
            for (int i = 0; i < PhoneBook.size(); i++)
                for (int j = i + 1; j < PhoneBook.size(); j++)
                    if (PhoneBook.get(i).getFirstName().equals(PhoneBook.get(j).getFirstName()) && PhoneBook.get(i).getContactPhoneNumber().equals(PhoneBook.get(j).getContactPhoneNumber())) {
                        System.out.println(PhoneBook.get(i).getFirstName() + PhoneBook.get(i).getContactPhoneNumber());
                        PhoneBook.remove(j);
    ...
    public void saveToTxtFile() {
        if (PhoneBook.size() > 0) {
            ...
            try {
                ...
                for (Contact c : PhoneBook)
                    wr.write(c.getFirstName() + "\t" + c.getContactPhoneNumber() + "\t" + c.getContactCompanyName() + "\n");
    ...
    public void LoadFileToPhoneBook(String path) {
        ...
        try {
            ...
            while ((strCurrentLine = bufReader.readLine()) != null) {
                Contact c = new Contact(strCurrentLine.split("\t")[0], strCurrentLine.split("\t")[1], strCurrentLine.split("\t")[2],strCurrentLine.split("\t")[3],strCurrentLine.split("\t")[4]);
                PhoneBook.add(c);
                flag = true;
            }
        } catch (Exception e) {
